const quiz = require('../../data/reverse.json');
...
module.exports = {
    ...
    run: async (message, args, client, photo) => {
        ...
        try {
            ...
            ctx.fillText('حاول كتابة الكلمة بشكل صحيح', 428, 135);
            ...
            await message.channel.send(file).then(() => {
                ...
                message.channel.awaitMessages(filter, { max: 1, time: 15000, errors: ["time"] })
                    .then(collected => {
                        ...
                        message.channel.send(`> **مبروك! الفائز هو ${collected.first().author} ، قام بكتابة الكلمة بالعكس قبل انتهاء الوقت بـ \`${(((date + 15000) - Date.now()) / 1000).toFixed(1)}\` ثواني!**`);
